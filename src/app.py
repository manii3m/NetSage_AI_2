    ...
    if f"diagnosis_{selected_case_id}" not in st.session_state:
        ...
        if checker_result["findings"]:
            for finding in checker_result["findings"]:
                ...
                st.markdown(textwrap.dedent(f"""<div style="border: 1px solid {border}; border-radius: 4px; padding: 16px; margin-bottom: 16px; background-color: {bg_card};">
...
<div style="font-size: 0.875rem; color: {text_main};">{finding['recommendation']}</div>
        ...
        st.markdown(textwrap.dedent(f"""<div class="minimal-card">
<div class="card-header">
AI Diagnosis
{demo_tag}
</div>
...
<div style="font-size: 0.8rem; color: {b_dang_tx}; font-family: 'Lora', serif; font-style: italic;">{ai_result.get("safety_note", "")}</div>
        ...
        if current_state == "PENDING":
            st.markdown(textwrap.dedent(f"""<div class="review-pending">
...
<div style="font-size: 0.875rem;">Please review the AI diagnosis before finalizing the case.</div>
    ...
    def apply_minimal_style(fig):
        fig.update_layout(
            ...
            xaxis=dict(showline=True, linewidth=1, linecolor=border_dark, gridcolor=border),
            yaxis=dict(showline=True, linewidth=1, linecolor=border_dark, gridcolor=border)
        )
        return fig
    ...
    if audit_log:
        ...
        with col_c1:
            ...
            color_map = {"CRITICAL": "#E63946", "HIGH": "#E63946", "MEDIUM": "#F4A261", "LOW": "#2A9D8F"}
            ...
            color_map2 = {"APPROVE": "#2A9D8F", "EDIT": "#F4A261", "REJECT": "#E63946"}
        ...
        with col_c2:
            ...
            fig2 = px.bar(concept_counts, y='Concept', x='Count', orientation='h', title="Cases by Concept", color_discrete_sequence=["#457B9D"])
            ...
            fig4 = px.bar(osi_counts, x='OSI Layer', y='Count', title="OSI Layer Distribution", color_discrete_sequence=["#8ECAE6"])
